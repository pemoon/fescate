            ...
            if ($node instanceof PhpParser\Node\Expr\FuncCall)      // processing function_exists('function_name');
            {
                if (isset($node->name->parts))                      // not set when indirect call (i.e.function name is a variable value!)
                {
                    ...
                    if ( is_string($name) && ($name=='function_exists') )
                    {
                        ...
                        if (!$ok)
                        {
                            throw new Exception("Error: your use of function_exists() function is not compatible with fescate!".PHP_EOL."\tOnly 1 literal string parameter is allowed...");
                        }
                        if ($warning) fprintf(STDERR, "Warning: your use of function_exists() function is not compatible with fescate!".PHP_EOL."\t Only 1 literal string parameter is allowed...".PHP_EOL);
                    }
                }
            }
        }
            ...
            if ($node instanceof PhpParser\Node\Expr\FuncCall)      // processing define('constant_name',value);
            {
                if (isset($node->name->parts))                      // not set when indirect call (i.e.function name is a variable value!)
                {
                    ...
                    if ( is_string($fn_name) && ( ($fn_name=='define') || ($fn_name=='defined') ) )
                    {
                        ...
                        if (!$ok)
                        {
                            if ($fn_name=='define') throw new Exception("Error: your use of $fn_name() function is not compatible with fescate!".PHP_EOL."\tOnly 2 parameters, when first is a literal string is allowed...");
                            else                    throw new Exception("Error: your use of $fn_name() function is not compatible with fescate!".PHP_EOL."\tOnly 1 literal string parameter is allowed...");
                        }
                    }
                }
            }
        ...
        if ($conf->obfuscate_loop_statement)                  // for while do while   are replaced by goto ...
        {
            ...
            if ($node instanceof PhpParser\Node\Stmt\Break_)
            {
                ...
                if (isset($node->num))
                {
                    ...
                    {
                        throw new Exception("Error: your use of break statement is not compatible with fescate!".PHP_EOL."\tAt max 1 literal numeric parameter is allowed...");
                    }
                }
            ...
            if ($node instanceof PhpParser\Node\Stmt\Continue_)
            {
                ...
                if (isset($node->num))
                {
                    ...
                    {
                        throw new Exception("Error: your use of continue statement is not compatible with fescate!".PHP_EOL."\tAt max 1 literal numeric parameter is allowed...");
                    }
                }
                if (count($this->t_loop_stack) - $n <0)
                {
