...
function check_preload_file($filename)                       // self-explanatory
{
    for($ok=false;;)
    {
        ...
        $line   = trim(fgets($fp));     if ($line!='// Fescate - Php Obfuscate: Preload File') { fclose($fp); break; }
        fclose($fp);
        $ok     = true;
        break;
    }
...
function check_config_file($filename)                       // self-explanatory
{
    for($ok=false;;)
    {
        ...
        $fp     = fopen($filename,"r"); if($fp===false) break;
        $line   = trim(fgets($fp));     if ($line!='<?php')                                     { fclose($fp); break; }
        $line   = trim(fgets($fp));     if ($line!='// Fescate - Php Obfuscate: Config File')  { fclose($fp); break; }
        fclose($fp);
        $ok     = true;
        break;
    }
