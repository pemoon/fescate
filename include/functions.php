...
function check_preload_file($filename)                       // self-explanatory
{
	for ($ok = false; ;) {
		...
		$line = trim(fgets($fp));
		if ($line != '// Fescate - Php Obfuscate: Preload File') {
			fclose($fp);
			break;
		}
		fclose($fp);
	...
	if (!$ok) fprintf(STDERR, "Warning:[%s] is not a valid fescate preload file!%s\tCheck if file is php, and if magic line is present!%s", $filename, PHP_EOL, PHP_EOL);
	return $ok;
}
...
//		$line = trim(fgets($fp));
//		if ($line != '// Fescate - Php Obfuscate: Config File') {
//			fclose($fp);
//			break;
//		}
		fclose($fp);
	...
	if (!$ok) fprintf(STDERR, "Warning:[%s] is not a valid fescate config file!%s\tCheck if file is php, and if magic line is present!%s", $filename, PHP_EOL, PHP_EOL);
	return $ok;
}

function create_context_directories($target_directory)      // self-explanatory
{
	foreach (["$target_directory/pemoon/fescate", "$target_directory/pemoon/fescate/context"] as $dummy => $dir) {
