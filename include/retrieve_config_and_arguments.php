...
$t_args                 = $argv;
$t_fescate_pathinfo   = pathinfo(realpath(array_shift($t_args)));
$fescate_dirname      = $t_fescate_pathinfo['dirname'];
...
{
    fprintf(STDERR,"Info:\tfescate version = %s%s",$fescate_version,PHP_EOL.PHP_EOL);
    $lang = '';
    if (($x = getenv('LANG'))!==false) $s = strtolower($x); $x = explode('_',$x); $x = $x[0];
         if (file_exists("$fescate_dirname/locale/$x/README.md"))  $help = file_get_contents("$fescate_dirname/locale/$x/README.md");
    else if (file_exists("$fescate_dirname/README.md"))            $help = file_get_contents("$fescate_dirname/README.md");
...
$config_file_namepart = 'fescate.cnf';    if (($x = getenv('FESCATE_CONFIG_FILENAME'))!==false) $config_file_namepart = $x;

                                                            $t_where    = array();
if ($argument_config_filename!='')                          $t_where[]  = $argument_config_filename;                        // --config-file argument
if (($x = getenv('FESCATE_CONFIG_FILE'))     !==false)    $t_where[]  = $x;                                               // FESCATE_CONFIG_FILE
if (($x = getenv('FESCATE_CONFIG_DIRECTORY'))!==false)    $t_where[]  = "$x/$config_file_namepart";                       // FESCATE_CONFIG_DIRECTORY
...
if ( $x                  !==false)                          $t_where[]  = "$x/config/$config_file_namepart";                // HOME/config
                                                            $t_where[]  = "/usr/local/pemoon/fescate/$config_file_namepart"; // /usr/local/pemoon/fescate
                                                            $t_where[]  = "$fescate_dirname/fescate.cnf";               // source_code_directory/default_conf_filename
...
if (!$conf->silent) fprintf(STDERR,"Info:\tfescate version = %s%s",$fescate_version,PHP_EOL);
            ...
            if (is_file($source_file) && is_readable($source_file))
            {
                ...
                if ( ($target_file!=='') && file_exists($target_file) )
                {
                    ...
                    if ( is_readable($x) && is_writable($x) && is_file($x) && (file_get_contents($x)!=='') )
                    {
                        ...
                        if (strpos($y,'    |  Obfuscated by Fescate - Php Obfuscate ')===false)       // comment is a magic string, used to not overwrite wrong files!!!
                        {
                            $x = realpath($target_file);
